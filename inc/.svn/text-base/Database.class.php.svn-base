<?php
require_once 'plogger/Logger.php';
...
function connect($new_link=false) {
	...
	if (!$this->link_id) {//open failed
		$this->logger->error("Could not connect to server:".$this->server);
		}
...
function query($sql) {
	...
	if (!$this->query_id) {
		$this->logger->error("MySQL Query fail: ".$sql);
		return 0;
	}
...
function fetch_array($query_id=-1) {
	...
	if (isset($this->query_id)) {
		...
		$this->logger->error("Invalid query_id: ".$this->query_id." Records could not be fetched.");
	}
